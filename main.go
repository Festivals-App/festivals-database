...
func sendDatabaseHeartbeat(conf *config.Config) {
	for {
		...
		var siteBeat *heartbeat.Heartbeat = &heartbeat.Heartbeat{Service: "festivals-database", Host: "mysql://" + conf.DatabaseBindAddress, Port: conf.DatabasePort, Available: true}
